...
class ModelAccountCustomer extends Model {
	...
	public function updateLogonAttempt($email){
		$this->db->query("UPDATE " . DB_PREFIX . "customer_login SET total = 1, date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "' WHERE email = '" . $this->db->escape(utf8_strtolower((string)$email)) . "' AND ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "'");
	}
}
